    ...
    <header class="rvt-header" role="banner">
        ...
        <div class="rvt-header__controls">
            ...
            <nav class="rvt-header__main-nav" role="navigation">
                ...
                <div class="rvt-modal" id="notification" role="dialog" aria-labelledby="modal-example-title" aria-hidden="true" tabindex=-1>
                    <div class="rvt-modal__inner">
                        <header class="rvt-modal__header">
                            <h1 class="rvt-modal__title" id="modal-example-title">Notification Details</h1>
                        </header>
                        <div class="rvt-modal__body">
                            <?php 
                                ...
                                $notification = mysqli_query($con, "SELECT * from Notification where notificationId = 16;");

                                while ($row = mysqli_fetch_assoc($notification)){
                                    $notiId = $row['notificationId'];
                                    $notiType = $row['notificationType'];
                                    $notiDate = $row['eventDate'];
                                    $notiLocation = $row['location'];
                                    $notiStart = $row['startTime'];
                                    $notiEnd = $row['endTime'];
                                    $notiDetails = $row['details'];
                                    $interacted = $row['interacted'];

                                    echo "<h2>Location: $notiLocation </h2>
                                    <h2>Date: $notiDate</h2>
                                    <h2>Start Time: $notiStart</h2>
                                    <h2>End Time: $notiEnd</h2>
                                    <h2>Details: $notiDetails</h2>";

                                }

                                
                            ?>
                            <!-- <h2>Location: Luddy Hall Main Lobby</h2>
                            <h2>Date: 3/30/2021</h2>
                            <h2>Start Time: 3:00 pm</h2>
                            <h2>End Time: 6:00 pm</h2>
                            <h2>Details: Networking event for Luddy students looking for summer internship opportunities.</h2> -->
                        </div>
                        <div class="rvt-modal__controls">
                            <button class="rvt-button"name="submit" value="notification">Accept</button>
                            <button type="button" class="rvt-button rvt-button--secondary" data-modal-close="notification">Dismiss</button>
                        </div>
                        <button type="button" class="rvt-button rvt-modal__close" data-modal-close="notification">
                            <span class="rvt-sr-only">Close</span>
                            <svg aria-hidden="true" xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 16 16">
                                <path fill="currentColor" d="M9.41,8l5.29-5.29a1,1,0,0,0-1.41-1.41L8,6.59,2.71,1.29A1,1,0,0,0,1.29,2.71L6.59,8,1.29,13.29a1,1,0,1,0,1.41,1.41L8,9.41l5.29,5.29a1,1,0,0,0,1.41-1.41Z"/>
                            </svg>
			...
			?>
                    ...
                    <div class="rvt-dropdown__menu rvt-header-id__menu" id="id-dropdown" aria-hidden="true">
                        ...
                        <div role="group" aria-label="User actions">
                            <a href="logout.php" onclick="signOut();">Log out</a>
                        </div>
                    </div>
                </div>
            </div>
...
<div class='all-body-container'>
        ...
        <form method="POST">
                    ...
                    <div class="rvt-flex rvt-wrap">
                        ...
                        <div class="rvt-width-md  group-name">
                                ...
                                <input type="text" id="input-4" name="group-name-form" value="">
                            </div>
                        ...
                        <div class="rvt-grow-1 rvt-m-right-sm rvt-wrap group-bio">
                                ...
                                <input type="text" class="rvt-wrap" id="input-5" name="group-biography" value="">
                            ...
                            <div class="rvt-dropdown rvt-width-md rvt-m-top-lg">
                                    <button type="button" class="rvt-button" data-dropdown-toggle="dropdown-navigation"
                                        ...
                                        <svg aria-hidden="true" class="rvt-m-left-xs" xmlns="http://www.w3.org/2000/svg"
                            ...
                            <!-- <div class="rvt-dropdown rvt-width-sm rvt-m-top-lg">
                            <button type="button" class="rvt-button" data-dropdown-toggle="dropdown-navigation"
                                        aria-haspopup="true" aria-expanded="false">
                            ...
                            <select id="group-names"  class ="rvt-button" name="group-type" required>
                        ...
                        <div class="rvt-flex rvt-m-top-lg save-button">
                            <input type="submit" name="submit" class="rvt-button" value="Done">
                        </div>
                        </div>
                        </div>
                    </div>
    ...
    <?php
        ...
        if ($_POST['submit'] =='Done') {
            ...
            $sql1 = "INSERT INTO Groups (groupName, groupType, biography, photo, pinnedChat)
            VALUES ('$gn', '$type','$bio',NULL, NULL);";
            // mysqli_query($con,$sql1);

            if (mysqli_query($con, $sql1)) {
                $last_group_id = mysqli_insert_id($con);
                $manager = "INSERT into Group_Managers (groupId, userId) Values ($last_group_id,'{$_SESSION['userId']}')";
                mysqli_query($con,$manager);

                $joining_group = "INSERT into User_Joined_Group (groupId, userId) Values ($last_group_id,'{$_SESSION['userId']}')";
                mysqli_query($con,$joining_group);

                if (!empty($checkbox1)) {
                    foreach($checkbox1 as $chk1)  {
                    // echo $chk1.'<br>';
                    $sql4 = "INSERT INTO Group_Interest (interestId,groupId) VALUES ($chk1,$last_group_id);";
            ...
            // $sql = "SELECT interest FROM Interest AS i JOIN Group_Interest AS gi ON gi.interestId=i.interestId JOIN Groups AS g ON gi.groupId=g.groupId WHERE g.groupId=1;";
            ...
            $con->close();
            echo "<script>window.location.href='viewGroup.php';</script>";
        }
    ?>
        </div>
        <!-- RIGHT BODY -->
