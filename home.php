                        ...
                        <ul>
                            ...
                            if (mysqli_num_rows($result) > 0) {
                                while ($row = mysqli_fetch_assoc($groups)) {
                                        $groupId = $row['groupId'];
                                        $group = $row['groupName'];
                                        echo '<input type="hidden" id="selectedGroup" name="selectedGroup" value="'.$groupId.'" />';
                                        echo '<button class="rvt-button rvt-button--plain">'.$group.'</button>';
...
<div class='all-body-container'>
    ...
    <div class="rvt-flex rvt-wrap">
        ...
        <div class="rvt-middle-body rvt-grow-1 rvt-m-right-sm">
            <div class="rvt-wrap rvt-box__box__body">
                <div class="rvt-container">
                    <form method="POST" action="createGroup.php">
                    <div class="rvt-button-group rvt-flex">
                            <input type="submit" class="rvt-button create-group" value="Create Group">
                    </div>
                    </form>
                    <div class="row-title">
                        <div class="col-title">
                            <h1 class="rvt-ts-xl">Favorited Groups</h1>
                            <br>
                        </div>
                    </div>
                    <div style="padding-left: 10px; padding-right: 10px;">
                        <!-- Favorited Groups Main Page Highlights -->
                        <?php
                        $con = mysqli_connect("db.sice.indiana.edu","i494f20_team20","my+sql=i494f20_team20", "i494f20_team20");
                        // Check Connection
                        if (!$con) {
                        ...
                        $favoritedGroups = mysqli_query($con, "SELECT LEFT(groupName,1) AS get_fname, g.groupId, g.groupName, g.biography, g.pinnedChat from Groups as g
                                        Right Join User_Favorited_Group as usg on usg.groupId = g.groupId
                                        where usg.UserId = '{$_SESSION['userId']}';");

                        if (mysqli_num_rows($favoritedGroups) > 0) {
                            while ($row = mysqli_fetch_assoc($favoritedGroups)) {
                                $get_fname = $row['get_fname'];
                                $groupId = $row['groupId'];
                                $groupName = $row['groupName'];
                                $biography = $row['biography'];
                                $pinnedChat = $row['pinnedChat'];

                                echo '<div class="rvt-box rvt-box--card">
                                <div class="rvt-box__body">';

                                echo '<div class="rvt-container">
                                        <div class="rvt-grid">
                                            <div class="rvt-grid__item-2-md-up">
                                                <div class="rvt-box__avatar2 update-photo2">
                                                    <div class="abbr-div2"><b class="abbr2">'.$get_fname.'</b></div>
                                                </div>
                                                <p style="padding-left: 40px">'.$groupName.'</p>
                                            </div>
                                            
                                            <div class="rvt-grid__item-5-md-up">
                                                <div class="rvt-grow-1 rvt-width-xxl rvt-m-top-lg rvt-m-right-sm rvt-wrap group-bio">
                                                <b>Group Bio:</b> '.$biography.'
                                                </br></br>';

                                                echo '<span class="rvt-badge rvt-badge--success-secondary"> Pinned Message: </b>';
                                                                
                                                if ($pinnedChat == "") {
                                                    echo "there is no current annoucement.";
                                                } else {
                                                    echo $pinnedChat;
                                                }
                                            ...
                                            <div class="rvt-grid__item-5-md-up">
                                                <div class="rvt-grow-1 rvt-width-xxl rvt-m-top-lg rvt-m-right-sm rvt-wrap group-bio">
                                                    <b>Recent Chat Activity: </b>';
                                    
                                    
                                    $sql = "select concat(u.fname,' ',u.lname) AS fullName,c.chatId, DATE_FORMAT(c.chatDate, '%M %d, %Y') as chatDate, TIME_FORMAT(c.chatTime, '%r') as chatTime,c.chatText,cc.userId,cc.groupId from Chat AS c 
                                    JOIN Created_Chat AS cc ON c.chatId=cc.chatId JOIN User AS u on u.userId= cc.userId WHERE groupId='$groupId' ORDER BY chatDate,chatTime desc LIMIT 2;";
                                ...
                                echo '<div class="rvt-button-group">
                                <button class="rvt-button rvt-button--plain rvt-button--small" onclick="ContentPage(this)" value="'.$groupId.'">View Chat</button>
                                </div>
                                    </div>
                                    </div><br>';
