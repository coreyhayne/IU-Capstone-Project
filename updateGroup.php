...
<div class='all-body-container'>
    ...
    <div class="rvt-flex rvt-wrap">
        ...
        <div class='rvt-hide-lg-down rvt-m-right-sm'>
            ...
            <div class="rvt-middle-body rvt-grow-1 rvt-m-right-sm home-body">
                <form method="POST">
                    <!-- <form method="POST" action="viewGroup.php?groupId=<?php echo $passedId; ?>"> -->
                    <div class="group-data">
                        <div class="rvt-flex rvt-wrap">
                            <div class="rvt-display-flex rvt-wrap rvt-width-md">
                                <div class="rvt-box__avatar update-photo">
                                    <?php
                                        ...
                                        $query = mysqli_query($con,"SELECT LEFT(groupName,1) AS get_fname from Groups WHERE groupId='$passedId'");
                                        // $result1= $con->query($get_fname);
                                        // $value1 = mysqli_fetch_object($result1);
                                        // $get_fname = $value1->get_fname;
                                        $fname_photo = "";
                                        if (mysqli_num_rows($query) > 0) {
                                            while($row = mysqli_fetch_assoc($query)) {
                                                $fname_photo = $row['get_fname'];
                                            }
                                        }

                                        echo '<div class="abbr-div"><b class="abbr">'.$fname_photo.'</b></div>';
                                    ?>
                                </div>
                                <div class="rvt-width-md  group-name">
                                    <label for="input-4">Group Name</label>
                                        <?php
                                            $con = mysqli_connect("db.sice.indiana.edu","i494f20_team20","my+sql=i494f20_team20", "i494f20_team20");
                                            // Check Connection
                                            if (!$con) {
                                            ...
                                            $sql = mysqli_query($con,"SELECT groupName FROM Groups WHERE groupId='$passedId'");
                                            $fname = "";
                                        ...
                                        ?>
                                    <input type="text" id="input-6" name="group-name-form" value="<?php echo $fname;?>">
                                </div>
                                <div class="rvt-display-flex">
                                    <div class="rvt-dropdown rvt-width-md rvt-m-top-lg">
                                        <button type="button" class="rvt-button"
                                            data-dropdown-toggle="dropdown-navigation2" aria-haspopup="true"
                                            aria-expanded="false">
                                            <span>Group Manager</span>
                                            <svg aria-hidden="true" class="rvt-m-left-xs"
                                        ...
                                        <div class="rvt-dropdown__menu" id="dropdown-navigation2" aria-hidden="true"
                                            role="menu">
                                            ...
                                            <?php
                                                ...
                                                $result = mysqli_query($con, "SELECT u.userId,concat(u.fname,' ',u.lname) AS fullName FROM User As u 
                                                                            Join User_Joined_Group As ug on ug.userId = u.userId 
                                                                            WHERE groupId ='$passedId'");
                                                $userArray = array();
                                                ...
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                    ...
                                                    $sql = mysqli_query($con,"SELECT u.userId, concat(u.fname,' ',u.lname) AS fullName FROM User As u Join Group_Managers As gm on gm.userId = u.userId WHERE gm.groupId ='$passedId'");
                                                    while($row = mysqli_fetch_assoc($sql)){
                                                        $username2 = $row['fullName']; 
                                                        $userId2 = $row['userId'];
                                                        if($userId == $userId2){
                                                            echo  '<li>
                                                            <input type="checkbox" name="usernames[]" id='.$userId2.' value='.$userId2.' checked>
                                                            <label for='.$userId2.' class="rvt-m-right-sm">'.$username2.'</label>
                                                            </li>';
                                                            array_push($userArray, $userId2);
                                                        }
                                                        
                                                    } if (!in_array($userId, $userArray)) {
                                                        echo  '<li>
                                                        <input type="checkbox" name="usernames[]" id='.$userId.' value='.$userId.'>
                                                        <label for='.$userId.' class="rvt-m-right-sm">'.$username.'</label>
                                                        </li>';
                                                    }    
                                                }  
                                                ?>
                            ...
                            <!-- <div class="rvt-display-flex rvt-wrap rvt-bg-green"> -->

                            <div class="rvt-grow-1 rvt-m-right-sm rvt-wrap group-bio">
                                <label for="input-5">Group Bio</label>
                                    <?php
                                    $con = mysqli_connect("db.sice.indiana.edu","i494f20_team20","my+sql=i494f20_team20", "i494f20_team20");
                                        // Check Connection
                                        if (!$con) {
                                        ...
                                        $sql = mysqli_query($con,"SELECT biography FROM Groups WHERE groupId='$passedId'");
                                        $biography = "";
                                        if (mysqli_num_rows($sql) > 0) {
                                            while($row = mysqli_fetch_assoc($sql)) {
                                    ...
                                    ?>
                                <textarea type="text" class="rvt-wrap rvt-m-bottom-md" id="input-5" name="group-biography"
                                    value=""><?php echo $biography;?></textarea>



                                    <div class="rvt-grow-1 rvt-m-right-sm rvt-wrap rvt-m-top-lg pinned-msg">
                                    <label for="textarea-success-state">Pinned Message:</label>
                                    <?php
                                            $con = mysqli_connect("db.sice.indiana.edu","i494f20_team20","my+sql=i494f20_team20", "i494f20_team20");
                                            // Check Connection
                                            if (!$con) {
                                        ...
                                        ?>  
                                    <textarea type="text" class="rvt-wrap rvt-m-bottom-md rvt-validation-success" id="textarea-success-state" name="group-pin-form" value=""><?php echo trim($pinchat); ?></textarea>
                                </div>
                                <div class="rvt-display-flex">
                                    <div class="rvt-dropdown rvt-width-md rvt-m-top-lg">
                                        <button type="button" class="rvt-button"
                                            data-dropdown-toggle="dropdown-navigation" aria-haspopup="true"
                                            aria-expanded="false">
                                        ...
                                        <div class="rvt-dropdown__menu" id="dropdown-navigation" aria-hidden="true"
                                            role="menu">
                                            ...
                                            <?php
                                                ...
                                                $result = mysqli_query($con, "SELECT interestId,interest from Interest;");
                                                $interestArray = array();
                                                ...
                                                while ($row = mysqli_fetch_assoc($result)) {
                                                    ...
                                                    $sql = mysqli_query($con,"SELECT interest FROM Interest AS i JOIN Group_Interest AS gi ON gi.interestId=i.interestId 
                                                                        JOIN Groups AS g ON gi.groupId=g.groupId WHERE g.groupId='$passedId'");
                                                    while($row = mysqli_fetch_assoc($sql)){
                                                        $interests2 = $row['interest']; 
                                                        if($interests == $interests2){
                                                            echo  '<li>
                                                            <input type="checkbox" name="interests[]" id='.$interestId.' value='.$interestId.' checked>
                                                            <label for='.$interestId.' class="rvt-m-right-sm">'.$interests.'</label>
                                                            </li>';
                                                            array_push($interestArray, $interests2);
                                                        }
                                                        
                                                    } if (!in_array($interests, $interestArray)) {
                                                        echo  '<li>
                                                        <input type="checkbox" name="interests[]" id='.$interestId.' value='.$interestId.'>
                                                        <label for='.$interestId.' class="rvt-m-right-sm">'.$interests.'</label>
                                                        </li>';
                                                    }    
                                                }  
                                    ...
                                    <div class="rvt-width-md rvt-m-top-lg rvt-bg-yellow">
                                        <select id="group-type" class="rvt-button" name="group-type" required>
                                            <option value="" disabled selected hidden>Group Type</option>
                                            <!-- <option value=""></option> -->
                                ...
                                <div class="rvt-flex rvt-m-top-lg save-button">
                                    <!-- <input type="submit" name="submit" class="rvt-button" value="Save"> -->
                                    <button class="rvt-button" name="submit" value="Save">Save
                                                </button>
                                </div>
                    ...
                    <?php 
                        ...
                        // if ($_POST['submit'] =='Save') {
                            ...
                            $gn =  mysqli_real_escape_string($con, $_POST['group-name-form']);
                            $bio =  mysqli_real_escape_string($con, $_POST['group-biography']);
                            $type = $_POST['group-type'];
                            $pin = $_POST['group-pin-form'];
                            // $tags = $_POST['form-interests'];
                            $checkbox1 = $_POST['interests'];
                            $checkbox2 = $_POST['usernames'];
                            
                            if (!empty($gn)) {
                            $sql = "UPDATE Groups SET groupName='$gn' WHERE groupId='$passedId'";
                            mysqli_query($con,$sql);
                            }
                            if (!empty($gn)) {
                                $sql1 = "UPDATE Groups SET biography='$bio' WHERE groupId='$passedId'";
                                mysqli_query($con,$sql1);   
                            }
                            if (!empty($type)) {
                                $sql2 = "UPDATE Groups SET groupType='$type' WHERE groupId='$passedId'";
                                mysqli_query($con,$sql2);
                                }
                            if (!empty($pin)) {
                                $sql10 = "UPDATE Groups SET pinnedChat='$pin' WHERE groupId='$passedId'";
                                mysqli_query($con,$sql10);
                                }
                            if (!empty($checkbox1)) {
                                $sql9 = "DELETE FROM Group_Interest WHERE groupId='$passedId'";
                                mysqli_query($con,$sql9);
                                foreach($checkbox1 as $chk1)  {
                                    // echo $chk1."<br>";
                                    $sql4 = "INSERT INTO Group_Interest (interestId,groupId) VALUES ($chk1,$passedId);";
                                    mysqli_query($con,$sql4);  
                                }
                            }
                            if (!empty($checkbox2)) {
                                $sql15 = "DELETE FROM Group_Managers WHERE groupId='$passedId'";
                                mysqli_query($con,$sql15);
                                
                                foreach($checkbox2 as $chk2)  
                                {   
                                    $sql19 = "INSERT INTO Group_Managers (groupId, userId) VALUES ($passedId, $chk2);";
                                    mysqli_query($con,$sql19);  
                                }
                                }
                            $con->close();
                            // header("Location: viewGroup.php?groupId=".$passedId);
                            echo "<script>window.location.href = 'viewGroup.php?groupId='+passedId;</script>";
                        }   
                    ?>
                </div>
            <!-- RIGHT BODY -->
    ...
    <script>
        function ContentPage2(elem) {
            window.location.href = "viewGroup.php?groupId=" + elem;
            return false;
        }
    </script>
