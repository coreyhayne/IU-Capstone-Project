<?php
    ...
    $eventsQuery = mysqli_query($con, "SELECT CONCAT(g.groupName,' ',n.notificationType,' Event') as title, DATE_FORMAT(TIMESTAMP(e.eventDate, ADDTIME(e.startTime, '4:00:00')), '%Y-%m-%dT%TZ') AS start, DATE_FORMAT(TIMESTAMP(e.eventDate, ADDTIME(e.endTime, '4:00:00')), '%Y-%m-%dT%TZ') AS end
    from Notification as n join Event as e on e.Id=n.eventId join Groups as g on g.groupId=n.groupId Where n.userId = '{$_SESSION['userId']}' and interacted = 'accepted'
    UNION
    Select CONCAT(g.groupName,' ',n.notificationType) as title, DATE_FORMAT(TIMESTAMP(crr.reservationDate, ADDTIME(crr.startTime, '4:00:00')), '%Y-%m-%dT%TZ') AS start, DATE_FORMAT(TIMESTAMP(crr.reservationDate, ADDTIME(crr.endTime, '4:00:00')), '%Y-%m-%dT%TZ') AS end
    ...
    join Groups as g on g.groupId=n.groupId Where n.userId = '{$_SESSION['userId']}' and interacted = 'accepted';");

    $events = array();
    while($rows = mysqli_fetch_assoc($eventsQuery)) {
        $events[] = $rows;
    }
    $eventsjson = json_encode($events);
    file_put_contents("fullcalendar/events.json", $eventsjson);
?>
