    ...
    <div class="rvt-flex rvt-wrap">
        ...
        <!-- MIDDLE BODY -->
            ...
            <div class="rvt-wrap rvt-box__body explore-flex">
                <h1 class="explore-groups-h1">Explore Users</h1>
                <?php
                $con = mysqli_connect("db.sice.indiana.edu","i494f20_team20","my+sql=i494f20_team20", "i494f20_team20");
                // Check Connection
                if (!$con) {
                ...
                $names = array();
                $ids = array();
                $sql = "SELECT concat(fname,' ',lname) AS fullName,userId from User where userId Not in(select userId from User where userId='{$_SESSION['userId']}')";
                ...
                if (mysqli_num_rows($result) > 0) {
                    ...
                    for($i = 0; $i < sizeof($names); $i++){
                        ...
                        $get_fname = "SELECT LEFT(fname,1) AS get_fname from User WHERE userId='$ids[$counter]'";
                        $get_lname = "SELECT LEFT(lname,1) AS get_lname from User WHERE userId='$ids[$counter]'";
                        $result1= $con->query($get_fname);
                                    $value1 = mysqli_fetch_object($result1);
                                    $get_fname = $value1->get_fname;
                        ...
                        $sql = "SELECT concat(fname,' ',lname) AS fullName FROM User WHERE userId='$ids[$counter]' LIMIT 1;";
                        ...
                        <p>Biography:';                 
                        $sql = "SELECT biography FROM User WHERE userId='$ids[$counter]'";
                        $result= $con->query($sql);
                        $value = mysqli_fetch_object($result);
                        ...
                        $sql = "SELECT interest FROM Interest AS i JOIN User_Interest AS ui ON ui.interestId=i.interestId 
                                JOIN User AS u ON ui.userId=u.userId WHERE u.userId='$ids[$counter]';";
                        ...
                        echo '</p>
                        <p>School Year:';
                        $sql = "SELECT class FROM User WHERE userId='$ids[$counter]';";
                        $result= $con->query($sql);
                        $value = mysqli_fetch_object($result);
                        $class = $value->class;
                        ...
                        if($_POST[$ids[$counter]] == 'Invite To Group'){
                            // echo $ids[$i];
                            $invite = "INSERT into Pending_User (pendingType, groupId, userId) Values ('invited',$group,'$ids[$counter]'));";
                            mysqli_query($con,$invite);
                        }
                        echo '</div>';
                        echo '
                        <div class="rvt-modal__controls">
                        <form method="POST">
                       <select id="group-names" name="form-group" required>
                       <option value="" disabled selected hidden>Choose A Group</option>';
                                        ...
                                        $result = mysqli_query($con,"SELECT g.groupId, g.groupName from Groups as g
                                                left join Group_Managers as gm on gm.groupId=g.groupId
                                   ...
                                   echo' </select>
                        <input type="submit" class="rvt-button" name='.$ids[$counter].' value="Invite To Group">';                           
                        echo ' </form></div>
